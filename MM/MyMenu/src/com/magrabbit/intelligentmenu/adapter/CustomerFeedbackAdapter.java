 ...
 * 
 */
public class CustomerFeedbackAdapter extends ArrayAdapter<CommentsEntity> {
	...
	private Process_loadRatingList mProcess;
	private float mRating;

	public CustomerFeedbackAdapter(Context context, int resource,
			List<CommentsEntity> lstFeebback, Process_loadRatingList process) {
		super(context, resource, lstFeebback);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		final CommentsEntity cusFeedback = getItem(position);
		if (cusFeedback == null) {
			return null;
		}
		...
		try {
			if (cusFeedback.mDateUpdated != null) {
				...
				if (!cusFeedback.mDateCreated.equals(cusFeedback.mDateUpdated)) {
					viewHolder.mTvDateTime.setText("edited on: "
							+ formattedDate);
				} else {
	...
	/**
	 * 
	 * @Description This task is to update comments of users after editing
	 */
	public class UpdateMenuItemCommentTask extends
			AsyncTask<CommentsEntity, Void, CommentUpdate> {
		...
		@Override
		protected CommentUpdate doInBackground(CommentsEntity... params) {
			try {
				CommentsEntity cusFeedBack = params[0];
				InputStream inputStream = Server.requestPost(ServerURL
						.getUrlUpdateMenuItemComment(cusFeedBack.mItemId),
						ServerURL.getEntityUpdateMenuItemComment(
	...
	public static abstract class Process_loadRatingList {
