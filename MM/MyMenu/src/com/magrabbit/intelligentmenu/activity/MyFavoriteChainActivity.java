...
import com.magrabbit.intelligentmenu.adapter.MyFavoriteChainAdapter;
...
import com.magrabbit.intelligentmenu.model.MyFavoriteChainEntity;
...
/**
 * @author vule
 * @Description This class use to load all favorites chain
 */
public class MyFavoriteChainActivity extends BaseActivity implements
		OnItemClickListener, OptionMenuClickListener {

	private List<MyFavoriteChainListEntity> mListFavoriteChains;
	private ListView mListView;
	private ImageView mIvLbSortName;
	private ImageView mIvLbSortAmount;
	private TextView mTvAmount;
	private TextView mTvSortName;
	private MyFavoriteChainAdapter mChainAdapter;
	private ImageView mIvResLogo;
	private MyFavoriteGlobalListEntity mGlobalObject;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_myfavorite_chain);
		...
		mListView = (ListView) findViewById(R.id.myfavorite_chain_list_lv);
		mIvLbSortName = (ImageView) findViewById(R.id.myfavorite_chain_iv_status_sortAphabel);
		mIvLbSortAmount = (ImageView) findViewById(R.id.myfavorite_chain_iv_status_sortAmount);
		mTvAmount = (TextView) findViewById(R.id.myfavorite_chain_sortAmount);
		mTvSortName = (TextView) findViewById(R.id.myfavorite_chain_sortAphabel);
		mIvResLogo = (ImageView) findViewById(R.id.myfavorite_chain_list_ic);
		...
		if (bGetGlobalỌbject != null) {
			mGlobalObject = getIntent().getExtras().getParcelable(
					StringExtraUtils.KEY_MYFAVORITE_GLOBAL);
		}

		try {
			mImageLoader.DisplayImage(ServerURL.URL + mGlobalObject.getLogo(),
					mIvResLogo);
		...
		new GetFavoriteChainTask().execute();
	...
	public void onClick_sortByAnphabelAZ() {
		try {
			...
			Collections.sort(mListFavoriteChains,
					new Comparator<MyFavoriteChainListEntity>() {
						@Override
						public int compare(MyFavoriteChainListEntity lhs,
								MyFavoriteChainListEntity rhs) {
							if (lhs.getName()
									.trim()
									.toUpperCase()
									.compareTo(
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		try {
			if (requestCode == CodeRequest.CODE_REQUEST_MYFAVORITE_CHAIN) {
	...
	public void onInviteClickListener() {
		...
		startActivityForResult(new Intent(this, InviteFriendActivity.class),
				CodeRequest.CODE_REQUEST_MYFAVORITE_CHAIN);
	}

	@Override
	public void onMyAccountClickListener() {
		startActivityForResult(new Intent(MyFavoriteChainActivity.this,
	...
	public void onStatsClickListener() {
		...
		DialogStatInfo dialog = new DialogStatInfo(
				MyFavoriteChainActivity.this, null, "", 0, true);
		dialog.show();
	...
	public void myFeedback() {
		try {
			mDialogMyFeedback = new DialogMyFeedback(
					MyFavoriteChainActivity.this, true, true,
					new ProcessDialogFeedback() {

						@Override
						public void click_Ok() {
							if (Utils
									.isNetworkConnected(MyFavoriteChainActivity.this)) {
								mDialogConfirmRating = new DialogConFirmRating(
										MyFavoriteChainActivity.this,
	...
	public void onItemClick(AdapterView<?> adapterView, View view, int postion,
			long id) {
		try {
			Intent intent = new Intent(MyFavoriteChainActivity.this,
					MyFavoriteRestaurantActivity.class);
			...
			if (!Utils.isNetworkConnected(this)) {
				...
				intent.putExtra(StringExtraUtils.KEY_MYFAVORITE_CHAIN,
						mListFavoriteChains.get(postion));
				if (intent != null) {
	...
	/**
	 * @author vule
	 * @description This asynctask will load all favorites of chain
	 * @result: list favorites of chain
	 * 
	 */
	public class GetFavoriteChainTask extends
			AsyncTask<String, Void, MyFavoriteChainEntity> {
		@Override
		...
		@Override
		protected MyFavoriteChainEntity doInBackground(String... params) {
			InputStream source = null;
			try {
				source = Server
						.requestGet(ServerURL.getUrlMyFavoriteChain(
								UserObject
										.getInstance(MyFavoriteChainActivity.this).mAccess_token,
								mGlobalObject.getChainName()));
				Gson gson = new Gson();
				String json = Server.inputStreamToString(source).toString();
				MyFavoriteChainEntity response = gson.fromJson(json,
						MyFavoriteChainEntity.class);
				return response;
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		@Override
		protected void onPostExecute(MyFavoriteChainEntity result) {
			super.onPostExecute(result);
			try {
				if (result == null) {
					if (!Utils.isNetworkConnected(MyFavoriteChainActivity.this)) {
						showToastMessage(getString(R.string.mess_error_network));
					} else {
						showToastMessage(MyFavoriteChainActivity.this
				...
				if (result.mError != null) {
					Toast.makeText(MyFavoriteChainActivity.this,
							result.mError.toString(), Toast.LENGTH_SHORT)
							.show();
					dismissProgress();
				} else {
					if (mListFavoriteChains == null) {
						mListFavoriteChains = new ArrayList<MyFavoriteChainListEntity>();
					}
					mListFavoriteChains.clear();

					int size = result.mListFavorites.size();
					if (size == 1) {
						MyFavoriteChainListEntity obj1 = result.mListFavorites
								.get(0);
						mListFavoriteChains.add(new MyFavoriteChainListEntity(
								obj1.mLocationId, obj1.mName, obj1.mAddress,
								obj1.mCity, obj1.mState, obj1.mZipcode,
								obj1.mMostRecent, obj1.mLogo,
						...
						for (int i = 0; i < size; i++) {
							...
							if (i == size - 1) {
								if (obj1.mTotalFavorites != -1) {
									mListFavoriteChains
											.add(new MyFavoriteChainListEntity(
													obj1.mLocationId,
													obj1.mName, obj1.mAddress,
													obj1.mCity, obj1.mState,
													obj1.mZipcode,
													obj1.mMostRecent,
													obj1.mLogo,
													obj1.mTotalFavorites));
								...
								for (int j = i + 1; j < size; j++) {
									MyFavoriteChainListEntity obj2 = result.mListFavorites
											.get(j);
									if ((obj1.mLocationId == obj2.mLocationId)
					...
					// mListFavoriteChains.addAll(result.mListFavorites);

					mChainAdapter = new MyFavoriteChainAdapter(
							MyFavoriteChainActivity.this,
							R.id.myfavorite_chain_list_lv, mListFavoriteChains);
